...
macro wraperror(code)
    ...
    if !$(esc(code))
        ...
        code = clingo_error_e(LibClingo.clingo_error_code())
        error("$code: $message")
    end
end
end
...
function clingo_error_string(code)
    ccall((:clingo_error_string, libclingo), Cstring, (clingo_error_t,), code)
end
...
function clingo_set_error(code, message)
    ccall((:clingo_set_error, libclingo), Cvoid, (clingo_error_t, Cstring), code, message)
end
...
function clingo_warning_string(code)
    ccall((:clingo_warning_string, libclingo), Cstring, (clingo_warning_t,), code)
end
...
function clingo_signature_create(name, arity, positive, signature)
    @wraperror ccall((:clingo_signature_create, libclingo), Bool, (Cstring, UInt32, Bool, Ptr{clingo_signature_t}), name, arity, positive, signature)
end

function clingo_signature_name(signature)
    ccall((:clingo_signature_name, libclingo), Cstring, (clingo_signature_t,), signature)
end
...
function clingo_signature_is_positive(signature)
    @wraperror ccall((:clingo_signature_is_positive, libclingo), Bool, (clingo_signature_t,), signature)
end

function clingo_signature_is_negative(signature)
    @wraperror ccall((:clingo_signature_is_negative, libclingo), Bool, (clingo_signature_t,), signature)
end

function clingo_signature_is_equal_to(a, b)
    @wraperror ccall((:clingo_signature_is_equal_to, libclingo), Bool, (clingo_signature_t, clingo_signature_t), a, b)
end

function clingo_signature_is_less_than(a, b)
    @wraperror ccall((:clingo_signature_is_less_than, libclingo), Bool, (clingo_signature_t, clingo_signature_t), a, b)
end
...
function clingo_symbol_create_string(string, symbol)
    @wraperror ccall((:clingo_symbol_create_string, libclingo), Bool, (Cstring, Ptr{clingo_symbol_t}), string, symbol)
end

function clingo_symbol_create_id(name, positive, symbol)
    @wraperror ccall((:clingo_symbol_create_id, libclingo), Bool, (Cstring, Bool, Ptr{clingo_symbol_t}), name, positive, symbol)
end

function clingo_symbol_create_function(name, arguments, arguments_size, positive, symbol)
    @wraperror ccall((:clingo_symbol_create_function, libclingo), Bool, (Cstring, Ptr{clingo_symbol_t}, Csize_t, Bool, Ptr{clingo_symbol_t}), name, arguments, arguments_size, positive, symbol)
end

function clingo_symbol_number(symbol, number)
    @wraperror ccall((:clingo_symbol_number, libclingo), Bool, (clingo_symbol_t, Ptr{Cint}), symbol, number)
end

function clingo_symbol_name(symbol, name)
    @wraperror ccall((:clingo_symbol_name, libclingo), Bool, (clingo_symbol_t, Ptr{Ptr{Cchar}}), symbol, name)
end

function clingo_symbol_string(symbol, string)
    @wraperror ccall((:clingo_symbol_string, libclingo), Bool, (clingo_symbol_t, Ptr{Ptr{Cchar}}), symbol, string)
end

function clingo_symbol_is_positive(symbol, positive)
    @wraperror ccall((:clingo_symbol_is_positive, libclingo), Bool, (clingo_symbol_t, Ptr{Bool}), symbol, positive)
end

function clingo_symbol_is_negative(symbol, negative)
    @wraperror ccall((:clingo_symbol_is_negative, libclingo), Bool, (clingo_symbol_t, Ptr{Bool}), symbol, negative)
end

function clingo_symbol_arguments(symbol, arguments, arguments_size)
    @wraperror ccall((:clingo_symbol_arguments, libclingo), Bool, (clingo_symbol_t, Ptr{Ptr{clingo_symbol_t}}, Ptr{Csize_t}), symbol, arguments, arguments_size)
end
...
function clingo_symbol_to_string_size(symbol, size)
    @wraperror ccall((:clingo_symbol_to_string_size, libclingo), Bool, (clingo_symbol_t, Ptr{Csize_t}), symbol, size)
end

function clingo_symbol_to_string(symbol, string, size)
    @wraperror ccall((:clingo_symbol_to_string, libclingo), Bool, (clingo_symbol_t, Cstring, Csize_t), symbol, string, size)
end

function clingo_symbol_is_equal_to(a, b)
    @wraperror ccall((:clingo_symbol_is_equal_to, libclingo), Bool, (clingo_symbol_t, clingo_symbol_t), a, b)
end

function clingo_symbol_is_less_than(a, b)
    @wraperror ccall((:clingo_symbol_is_less_than, libclingo), Bool, (clingo_symbol_t, clingo_symbol_t), a, b)
end
...
function clingo_parse_term(string, logger, logger_data, message_limit, symbol)
    @wraperror ccall((:clingo_parse_term, libclingo), Bool, (Cstring, clingo_logger_t, Ptr{Cvoid}, Cuint, Ptr{clingo_symbol_t}), string, logger, logger_data, message_limit, symbol)
end
...
function clingo_symbolic_atoms_size(atoms, size)
    @wraperror ccall((:clingo_symbolic_atoms_size, libclingo), Bool, (Ptr{clingo_symbolic_atoms_t}, Ptr{Csize_t}), atoms, size)
end

function clingo_symbolic_atoms_begin(atoms, signature, iterator)
    @wraperror ccall((:clingo_symbolic_atoms_begin, libclingo), Bool, (Ptr{clingo_symbolic_atoms_t}, Ptr{clingo_signature_t}, Ptr{clingo_symbolic_atom_iterator_t}), atoms, signature, iterator)
end

function clingo_symbolic_atoms_end(atoms, iterator)
    @wraperror ccall((:clingo_symbolic_atoms_end, libclingo), Bool, (Ptr{clingo_symbolic_atoms_t}, Ptr{clingo_symbolic_atom_iterator_t}), atoms, iterator)
end

function clingo_symbolic_atoms_find(atoms, symbol, iterator)
    @wraperror ccall((:clingo_symbolic_atoms_find, libclingo), Bool, (Ptr{clingo_symbolic_atoms_t}, clingo_symbol_t, Ptr{clingo_symbolic_atom_iterator_t}), atoms, symbol, iterator)
end

function clingo_symbolic_atoms_iterator_is_equal_to(atoms, a, b, equal)
    @wraperror ccall((:clingo_symbolic_atoms_iterator_is_equal_to, libclingo), Bool, (Ptr{clingo_symbolic_atoms_t}, clingo_symbolic_atom_iterator_t, clingo_symbolic_atom_iterator_t, Ptr{Bool}), atoms, a, b, equal)
end

function clingo_symbolic_atoms_symbol(atoms, iterator, symbol)
    @wraperror ccall((:clingo_symbolic_atoms_symbol, libclingo), Bool, (Ptr{clingo_symbolic_atoms_t}, clingo_symbolic_atom_iterator_t, Ptr{clingo_symbol_t}), atoms, iterator, symbol)
end

function clingo_symbolic_atoms_is_fact(atoms, iterator, fact)
    @wraperror ccall((:clingo_symbolic_atoms_is_fact, libclingo), Bool, (Ptr{clingo_symbolic_atoms_t}, clingo_symbolic_atom_iterator_t, Ptr{Bool}), atoms, iterator, fact)
end

function clingo_symbolic_atoms_is_external(atoms, iterator, external)
    @wraperror ccall((:clingo_symbolic_atoms_is_external, libclingo), Bool, (Ptr{clingo_symbolic_atoms_t}, clingo_symbolic_atom_iterator_t, Ptr{Bool}), atoms, iterator, external)
end

function clingo_symbolic_atoms_literal(atoms, iterator, literal)
    @wraperror ccall((:clingo_symbolic_atoms_literal, libclingo), Bool, (Ptr{clingo_symbolic_atoms_t}, clingo_symbolic_atom_iterator_t, Ptr{clingo_literal_t}), atoms, iterator, literal)
end

function clingo_symbolic_atoms_signatures_size(atoms, size)
    @wraperror ccall((:clingo_symbolic_atoms_signatures_size, libclingo), Bool, (Ptr{clingo_symbolic_atoms_t}, Ptr{Csize_t}), atoms, size)
end

function clingo_symbolic_atoms_signatures(atoms, signatures, size)
    @wraperror ccall((:clingo_symbolic_atoms_signatures, libclingo), Bool, (Ptr{clingo_symbolic_atoms_t}, Ptr{clingo_signature_t}, Csize_t), atoms, signatures, size)
end

function clingo_symbolic_atoms_next(atoms, iterator, next)
    @wraperror ccall((:clingo_symbolic_atoms_next, libclingo), Bool, (Ptr{clingo_symbolic_atoms_t}, clingo_symbolic_atom_iterator_t, Ptr{clingo_symbolic_atom_iterator_t}), atoms, iterator, next)
end

function clingo_symbolic_atoms_is_valid(atoms, iterator, valid)
    @wraperror ccall((:clingo_symbolic_atoms_is_valid, libclingo), Bool, (Ptr{clingo_symbolic_atoms_t}, clingo_symbolic_atom_iterator_t, Ptr{Bool}), atoms, iterator, valid)
end
...
function clingo_theory_atoms_term_type(atoms, term, type)
    @wraperror ccall((:clingo_theory_atoms_term_type, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, clingo_id_t, Ptr{clingo_theory_term_type_t}), atoms, term, type)
end

function clingo_theory_atoms_term_number(atoms, term, number)
    @wraperror ccall((:clingo_theory_atoms_term_number, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, clingo_id_t, Ptr{Cint}), atoms, term, number)
end

function clingo_theory_atoms_term_name(atoms, term, name)
    @wraperror ccall((:clingo_theory_atoms_term_name, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, clingo_id_t, Ptr{Ptr{Cchar}}), atoms, term, name)
end

function clingo_theory_atoms_term_arguments(atoms, term, arguments, size)
    @wraperror ccall((:clingo_theory_atoms_term_arguments, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, clingo_id_t, Ptr{Ptr{clingo_id_t}}, Ptr{Csize_t}), atoms, term, arguments, size)
end

function clingo_theory_atoms_term_to_string_size(atoms, term, size)
    @wraperror ccall((:clingo_theory_atoms_term_to_string_size, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, clingo_id_t, Ptr{Csize_t}), atoms, term, size)
end

function clingo_theory_atoms_term_to_string(atoms, term, string, size)
    @wraperror ccall((:clingo_theory_atoms_term_to_string, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, clingo_id_t, Cstring, Csize_t), atoms, term, string, size)
end

function clingo_theory_atoms_element_tuple(atoms, element, tuple, size)
    @wraperror ccall((:clingo_theory_atoms_element_tuple, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, clingo_id_t, Ptr{Ptr{clingo_id_t}}, Ptr{Csize_t}), atoms, element, tuple, size)
end

function clingo_theory_atoms_element_condition(atoms, element, condition, size)
    @wraperror ccall((:clingo_theory_atoms_element_condition, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, clingo_id_t, Ptr{Ptr{clingo_literal_t}}, Ptr{Csize_t}), atoms, element, condition, size)
end

function clingo_theory_atoms_element_condition_id(atoms, element, condition)
    @wraperror ccall((:clingo_theory_atoms_element_condition_id, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, clingo_id_t, Ptr{clingo_literal_t}), atoms, element, condition)
end

function clingo_theory_atoms_element_to_string_size(atoms, element, size)
    @wraperror ccall((:clingo_theory_atoms_element_to_string_size, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, clingo_id_t, Ptr{Csize_t}), atoms, element, size)
end

function clingo_theory_atoms_element_to_string(atoms, element, string, size)
    @wraperror ccall((:clingo_theory_atoms_element_to_string, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, clingo_id_t, Cstring, Csize_t), atoms, element, string, size)
end

function clingo_theory_atoms_size(atoms, size)
    @wraperror ccall((:clingo_theory_atoms_size, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, Ptr{Csize_t}), atoms, size)
end

function clingo_theory_atoms_atom_term(atoms, atom, term)
    @wraperror ccall((:clingo_theory_atoms_atom_term, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, clingo_id_t, Ptr{clingo_id_t}), atoms, atom, term)
end

function clingo_theory_atoms_atom_elements(atoms, atom, elements, size)
    @wraperror ccall((:clingo_theory_atoms_atom_elements, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, clingo_id_t, Ptr{Ptr{clingo_id_t}}, Ptr{Csize_t}), atoms, atom, elements, size)
end

function clingo_theory_atoms_atom_has_guard(atoms, atom, has_guard)
    @wraperror ccall((:clingo_theory_atoms_atom_has_guard, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, clingo_id_t, Ptr{Bool}), atoms, atom, has_guard)
end

function clingo_theory_atoms_atom_guard(atoms, atom, connective, term)
    @wraperror ccall((:clingo_theory_atoms_atom_guard, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, clingo_id_t, Ptr{Ptr{Cchar}}, Ptr{clingo_id_t}), atoms, atom, connective, term)
end

function clingo_theory_atoms_atom_literal(atoms, atom, literal)
    @wraperror ccall((:clingo_theory_atoms_atom_literal, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, clingo_id_t, Ptr{clingo_literal_t}), atoms, atom, literal)
end

function clingo_theory_atoms_atom_to_string_size(atoms, atom, size)
    @wraperror ccall((:clingo_theory_atoms_atom_to_string_size, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, clingo_id_t, Ptr{Csize_t}), atoms, atom, size)
end

function clingo_theory_atoms_atom_to_string(atoms, atom, string, size)
    @wraperror ccall((:clingo_theory_atoms_atom_to_string, libclingo), Bool, (Ptr{clingo_theory_atoms_t}, clingo_id_t, Cstring, Csize_t), atoms, atom, string, size)
end
...
function clingo_assignment_has_conflict(assignment)
    @wraperror ccall((:clingo_assignment_has_conflict, libclingo), Bool, (Ptr{clingo_assignment_t},), assignment)
end

function clingo_assignment_has_literal(assignment, literal)
    @wraperror ccall((:clingo_assignment_has_literal, libclingo), Bool, (Ptr{clingo_assignment_t}, clingo_literal_t), assignment, literal)
end

function clingo_assignment_level(assignment, literal, level)
    @wraperror ccall((:clingo_assignment_level, libclingo), Bool, (Ptr{clingo_assignment_t}, clingo_literal_t, Ptr{UInt32}), assignment, literal, level)
end

function clingo_assignment_decision(assignment, level, literal)
    @wraperror ccall((:clingo_assignment_decision, libclingo), Bool, (Ptr{clingo_assignment_t}, UInt32, Ptr{clingo_literal_t}), assignment, level, literal)
end

function clingo_assignment_is_fixed(assignment, literal, is_fixed)
    @wraperror ccall((:clingo_assignment_is_fixed, libclingo), Bool, (Ptr{clingo_assignment_t}, clingo_literal_t, Ptr{Bool}), assignment, literal, is_fixed)
end

function clingo_assignment_is_true(assignment, literal, is_true)
    @wraperror ccall((:clingo_assignment_is_true, libclingo), Bool, (Ptr{clingo_assignment_t}, clingo_literal_t, Ptr{Bool}), assignment, literal, is_true)
end

function clingo_assignment_is_false(assignment, literal, is_false)
    @wraperror ccall((:clingo_assignment_is_false, libclingo), Bool, (Ptr{clingo_assignment_t}, clingo_literal_t, Ptr{Bool}), assignment, literal, is_false)
end

function clingo_assignment_truth_value(assignment, literal, value)
    @wraperror ccall((:clingo_assignment_truth_value, libclingo), Bool, (Ptr{clingo_assignment_t}, clingo_literal_t, Ptr{clingo_truth_value_t}), assignment, literal, value)
end
...
function clingo_assignment_at(assignment, offset, literal)
    @wraperror ccall((:clingo_assignment_at, libclingo), Bool, (Ptr{clingo_assignment_t}, Csize_t, Ptr{clingo_literal_t}), assignment, offset, literal)
end

function clingo_assignment_is_total(assignment)
    @wraperror ccall((:clingo_assignment_is_total, libclingo), Bool, (Ptr{clingo_assignment_t},), assignment)
end

function clingo_assignment_trail_size(assignment, size)
    @wraperror ccall((:clingo_assignment_trail_size, libclingo), Bool, (Ptr{clingo_assignment_t}, Ptr{UInt32}), assignment, size)
end

function clingo_assignment_trail_begin(assignment, level, offset)
    @wraperror ccall((:clingo_assignment_trail_begin, libclingo), Bool, (Ptr{clingo_assignment_t}, UInt32, Ptr{UInt32}), assignment, level, offset)
end

function clingo_assignment_trail_end(assignment, level, offset)
    @wraperror ccall((:clingo_assignment_trail_end, libclingo), Bool, (Ptr{clingo_assignment_t}, UInt32, Ptr{UInt32}), assignment, level, offset)
end

function clingo_assignment_trail_at(assignment, offset, literal)
    @wraperror ccall((:clingo_assignment_trail_at, libclingo), Bool, (Ptr{clingo_assignment_t}, UInt32, Ptr{clingo_literal_t}), assignment, offset, literal)
end
...
function clingo_propagate_init_solver_literal(init, aspif_literal, solver_literal)
    @wraperror ccall((:clingo_propagate_init_solver_literal, libclingo), Bool, (Ptr{clingo_propagate_init_t}, clingo_literal_t, Ptr{clingo_literal_t}), init, aspif_literal, solver_literal)
end

function clingo_propagate_init_add_watch(init, solver_literal)
    @wraperror ccall((:clingo_propagate_init_add_watch, libclingo), Bool, (Ptr{clingo_propagate_init_t}, clingo_literal_t), init, solver_literal)
end

function clingo_propagate_init_add_watch_to_thread(init, solver_literal, thread_id)
    @wraperror ccall((:clingo_propagate_init_add_watch_to_thread, libclingo), Bool, (Ptr{clingo_propagate_init_t}, clingo_literal_t, clingo_id_t), init, solver_literal, thread_id)
end

function clingo_propagate_init_remove_watch(init, solver_literal)
    @wraperror ccall((:clingo_propagate_init_remove_watch, libclingo), Bool, (Ptr{clingo_propagate_init_t}, clingo_literal_t), init, solver_literal)
end

function clingo_propagate_init_remove_watch_from_thread(init, solver_literal, thread_id)
    @wraperror ccall((:clingo_propagate_init_remove_watch_from_thread, libclingo), Bool, (Ptr{clingo_propagate_init_t}, clingo_literal_t, UInt32), init, solver_literal, thread_id)
end

function clingo_propagate_init_freeze_literal(init, solver_literal)
    @wraperror ccall((:clingo_propagate_init_freeze_literal, libclingo), Bool, (Ptr{clingo_propagate_init_t}, clingo_literal_t), init, solver_literal)
end

function clingo_propagate_init_symbolic_atoms(init, atoms)
    @wraperror ccall((:clingo_propagate_init_symbolic_atoms, libclingo), Bool, (Ptr{clingo_propagate_init_t}, Ptr{Ptr{clingo_symbolic_atoms_t}}), init, atoms)
end

function clingo_propagate_init_theory_atoms(init, atoms)
    @wraperror ccall((:clingo_propagate_init_theory_atoms, libclingo), Bool, (Ptr{clingo_propagate_init_t}, Ptr{Ptr{clingo_theory_atoms_t}}), init, atoms)
end
...
function clingo_propagate_init_add_literal(init, freeze, result)
    @wraperror ccall((:clingo_propagate_init_add_literal, libclingo), Bool, (Ptr{clingo_propagate_init_t}, Bool, Ptr{clingo_literal_t}), init, freeze, result)
end

function clingo_propagate_init_add_clause(init, clause, size, result)
    @wraperror ccall((:clingo_propagate_init_add_clause, libclingo), Bool, (Ptr{clingo_propagate_init_t}, Ptr{clingo_literal_t}, Csize_t, Ptr{Bool}), init, clause, size, result)
end

function clingo_propagate_init_add_weight_constraint(init, literal, literals, size, bound, type, compare_equal, result)
    @wraperror ccall((:clingo_propagate_init_add_weight_constraint, libclingo), Bool, (Ptr{clingo_propagate_init_t}, clingo_literal_t, Ptr{clingo_weighted_literal_t}, Csize_t, clingo_weight_t, clingo_weight_constraint_type_t, Bool, Ptr{Bool}), init, literal, literals, size, bound, type, compare_equal, result)
end

function clingo_propagate_init_add_minimize(init, literal, weight, priority)
    @wraperror ccall((:clingo_propagate_init_add_minimize, libclingo), Bool, (Ptr{clingo_propagate_init_t}, clingo_literal_t, clingo_weight_t, clingo_weight_t), init, literal, weight, priority)
end

function clingo_propagate_init_propagate(init, result)
    @wraperror ccall((:clingo_propagate_init_propagate, libclingo), Bool, (Ptr{clingo_propagate_init_t}, Ptr{Bool}), init, result)
end
...
function clingo_propagate_control_add_literal(control, result)
    @wraperror ccall((:clingo_propagate_control_add_literal, libclingo), Bool, (Ptr{clingo_propagate_control_t}, Ptr{clingo_literal_t}), control, result)
end

function clingo_propagate_control_add_watch(control, literal)
    @wraperror ccall((:clingo_propagate_control_add_watch, libclingo), Bool, (Ptr{clingo_propagate_control_t}, clingo_literal_t), control, literal)
end

function clingo_propagate_control_has_watch(control, literal)
    @wraperror ccall((:clingo_propagate_control_has_watch, libclingo), Bool, (Ptr{clingo_propagate_control_t}, clingo_literal_t), control, literal)
end
...
function clingo_propagate_control_add_clause(control, clause, size, type, result)
    @wraperror ccall((:clingo_propagate_control_add_clause, libclingo), Bool, (Ptr{clingo_propagate_control_t}, Ptr{clingo_literal_t}, Csize_t, clingo_clause_type_t, Ptr{Bool}), control, clause, size, type, result)
end

function clingo_propagate_control_propagate(control, result)
    @wraperror ccall((:clingo_propagate_control_propagate, libclingo), Bool, (Ptr{clingo_propagate_control_t}, Ptr{Bool}), control, result)
end
...
function clingo_backend_begin(backend)
    @wraperror ccall((:clingo_backend_begin, libclingo), Bool, (Ptr{clingo_backend_t},), backend)
end

function clingo_backend_end(backend)
    @wraperror ccall((:clingo_backend_end, libclingo), Bool, (Ptr{clingo_backend_t},), backend)
end

function clingo_backend_rule(backend, choice, head, head_size, body, body_size)
    @wraperror ccall((:clingo_backend_rule, libclingo), Bool, (Ptr{clingo_backend_t}, Bool, Ptr{clingo_atom_t}, Csize_t, Ptr{clingo_literal_t}, Csize_t), backend, choice, head, head_size, body, body_size)
end

function clingo_backend_weight_rule(backend, choice, head, head_size, lower_bound, body, body_size)
    @wraperror ccall((:clingo_backend_weight_rule, libclingo), Bool, (Ptr{clingo_backend_t}, Bool, Ptr{clingo_atom_t}, Csize_t, clingo_weight_t, Ptr{clingo_weighted_literal_t}, Csize_t), backend, choice, head, head_size, lower_bound, body, body_size)
end

function clingo_backend_minimize(backend, priority, literals, size)
    @wraperror ccall((:clingo_backend_minimize, libclingo), Bool, (Ptr{clingo_backend_t}, clingo_weight_t, Ptr{clingo_weighted_literal_t}, Csize_t), backend, priority, literals, size)
end

function clingo_backend_project(backend, atoms, size)
    @wraperror ccall((:clingo_backend_project, libclingo), Bool, (Ptr{clingo_backend_t}, Ptr{clingo_atom_t}, Csize_t), backend, atoms, size)
end

function clingo_backend_external(backend, atom, type)
    @wraperror ccall((:clingo_backend_external, libclingo), Bool, (Ptr{clingo_backend_t}, clingo_atom_t, clingo_external_type_t), backend, atom, type)
end

function clingo_backend_assume(backend, literals, size)
    @wraperror ccall((:clingo_backend_assume, libclingo), Bool, (Ptr{clingo_backend_t}, Ptr{clingo_literal_t}, Csize_t), backend, literals, size)
end

function clingo_backend_heuristic(backend, atom, type, bias, priority, condition, size)
    @wraperror ccall((:clingo_backend_heuristic, libclingo), Bool, (Ptr{clingo_backend_t}, clingo_atom_t, clingo_heuristic_type_t, Cint, Cuint, Ptr{clingo_literal_t}, Csize_t), backend, atom, type, bias, priority, condition, size)
end

function clingo_backend_acyc_edge(backend, node_u, node_v, condition, size)
    @wraperror ccall((:clingo_backend_acyc_edge, libclingo), Bool, (Ptr{clingo_backend_t}, Cint, Cint, Ptr{clingo_literal_t}, Csize_t), backend, node_u, node_v, condition, size)
end

function clingo_backend_add_atom(backend, symbol, atom)
    @wraperror ccall((:clingo_backend_add_atom, libclingo), Bool, (Ptr{clingo_backend_t}, Ptr{clingo_symbol_t}, Ptr{clingo_atom_t}), backend, symbol, atom)
end
...
function clingo_configuration_root(configuration, key)
    @wraperror ccall((:clingo_configuration_root, libclingo), Bool, (Ptr{clingo_configuration_t}, Ptr{clingo_id_t}), configuration, key)
end

function clingo_configuration_type(configuration, key, type)
    @wraperror ccall((:clingo_configuration_type, libclingo), Bool, (Ptr{clingo_configuration_t}, clingo_id_t, Ptr{clingo_configuration_type_bitset_t}), configuration, key, type)
end

function clingo_configuration_description(configuration, key, description)
    @wraperror ccall((:clingo_configuration_description, libclingo), Bool, (Ptr{clingo_configuration_t}, clingo_id_t, Ptr{Ptr{Cchar}}), configuration, key, description)
end

function clingo_configuration_array_size(configuration, key, size)
    @wraperror ccall((:clingo_configuration_array_size, libclingo), Bool, (Ptr{clingo_configuration_t}, clingo_id_t, Ptr{Csize_t}), configuration, key, size)
end

function clingo_configuration_array_at(configuration, key, offset, subkey)
    @wraperror ccall((:clingo_configuration_array_at, libclingo), Bool, (Ptr{clingo_configuration_t}, clingo_id_t, Csize_t, Ptr{clingo_id_t}), configuration, key, offset, subkey)
end

function clingo_configuration_map_size(configuration, key, size)
    @wraperror ccall((:clingo_configuration_map_size, libclingo), Bool, (Ptr{clingo_configuration_t}, clingo_id_t, Ptr{Csize_t}), configuration, key, size)
end

function clingo_configuration_map_has_subkey(configuration, key, name, result)
    @wraperror ccall((:clingo_configuration_map_has_subkey, libclingo), Bool, (Ptr{clingo_configuration_t}, clingo_id_t, Cstring, Ptr{Bool}), configuration, key, name, result)
end

function clingo_configuration_map_subkey_name(configuration, key, offset, name)
    @wraperror ccall((:clingo_configuration_map_subkey_name, libclingo), Bool, (Ptr{clingo_configuration_t}, clingo_id_t, Csize_t, Ptr{Ptr{Cchar}}), configuration, key, offset, name)
end

function clingo_configuration_map_at(configuration, key, name, subkey)
    @wraperror ccall((:clingo_configuration_map_at, libclingo), Bool, (Ptr{clingo_configuration_t}, clingo_id_t, Cstring, Ptr{clingo_id_t}), configuration, key, name, subkey)
end

function clingo_configuration_value_is_assigned(configuration, key, assigned)
    @wraperror ccall((:clingo_configuration_value_is_assigned, libclingo), Bool, (Ptr{clingo_configuration_t}, clingo_id_t, Ptr{Bool}), configuration, key, assigned)
end

function clingo_configuration_value_get_size(configuration, key, size)
    @wraperror ccall((:clingo_configuration_value_get_size, libclingo), Bool, (Ptr{clingo_configuration_t}, clingo_id_t, Ptr{Csize_t}), configuration, key, size)
end

function clingo_configuration_value_get(configuration, key, value, size)
    @wraperror ccall((:clingo_configuration_value_get, libclingo), Bool, (Ptr{clingo_configuration_t}, clingo_id_t, Cstring, Csize_t), configuration, key, value, size)
end

function clingo_configuration_value_set(configuration, key, value)
    @wraperror ccall((:clingo_configuration_value_set, libclingo), Bool, (Ptr{clingo_configuration_t}, clingo_id_t, Cstring), configuration, key, value)
end
...
function clingo_statistics_root(statistics, key)
    @wraperror ccall((:clingo_statistics_root, libclingo), Bool, (Ptr{clingo_statistics_t}, Ptr{UInt64}), statistics, key)
end

function clingo_statistics_type(statistics, key, type)
    @wraperror ccall((:clingo_statistics_type, libclingo), Bool, (Ptr{clingo_statistics_t}, UInt64, Ptr{clingo_statistics_type_t}), statistics, key, type)
end

function clingo_statistics_array_size(statistics, key, size)
    @wraperror ccall((:clingo_statistics_array_size, libclingo), Bool, (Ptr{clingo_statistics_t}, UInt64, Ptr{Csize_t}), statistics, key, size)
end

function clingo_statistics_array_at(statistics, key, offset, subkey)
    @wraperror ccall((:clingo_statistics_array_at, libclingo), Bool, (Ptr{clingo_statistics_t}, UInt64, Csize_t, Ptr{UInt64}), statistics, key, offset, subkey)
end

function clingo_statistics_array_push(statistics, key, type, subkey)
    @wraperror ccall((:clingo_statistics_array_push, libclingo), Bool, (Ptr{clingo_statistics_t}, UInt64, clingo_statistics_type_t, Ptr{UInt64}), statistics, key, type, subkey)
end

function clingo_statistics_map_size(statistics, key, size)
    @wraperror ccall((:clingo_statistics_map_size, libclingo), Bool, (Ptr{clingo_statistics_t}, UInt64, Ptr{Csize_t}), statistics, key, size)
end

function clingo_statistics_map_has_subkey(statistics, key, name, result)
    @wraperror ccall((:clingo_statistics_map_has_subkey, libclingo), Bool, (Ptr{clingo_statistics_t}, UInt64, Cstring, Ptr{Bool}), statistics, key, name, result)
end

function clingo_statistics_map_subkey_name(statistics, key, offset, name)
    @wraperror ccall((:clingo_statistics_map_subkey_name, libclingo), Bool, (Ptr{clingo_statistics_t}, UInt64, Csize_t, Ptr{Ptr{Cchar}}), statistics, key, offset, name)
end

function clingo_statistics_map_at(statistics, key, name, subkey)
    @wraperror ccall((:clingo_statistics_map_at, libclingo), Bool, (Ptr{clingo_statistics_t}, UInt64, Cstring, Ptr{UInt64}), statistics, key, name, subkey)
end

function clingo_statistics_map_add_subkey(statistics, key, name, type, subkey)
    @wraperror ccall((:clingo_statistics_map_add_subkey, libclingo), Bool, (Ptr{clingo_statistics_t}, UInt64, Cstring, clingo_statistics_type_t, Ptr{UInt64}), statistics, key, name, type, subkey)
end

function clingo_statistics_value_get(statistics, key, value)
    @wraperror ccall((:clingo_statistics_value_get, libclingo), Bool, (Ptr{clingo_statistics_t}, UInt64, Ptr{Cdouble}), statistics, key, value)
end

function clingo_statistics_value_set(statistics, key, value)
    @wraperror ccall((:clingo_statistics_value_set, libclingo), Bool, (Ptr{clingo_statistics_t}, UInt64, Cdouble), statistics, key, value)
end
...
function clingo_model_type(model, type)
    @wraperror ccall((:clingo_model_type, libclingo), Bool, (Ptr{clingo_model_t}, Ptr{clingo_model_type_t}), model, type)
end

function clingo_model_number(model, number)
    @wraperror ccall((:clingo_model_number, libclingo), Bool, (Ptr{clingo_model_t}, Ptr{UInt64}), model, number)
end

function clingo_model_symbols_size(model, show, size)
    @wraperror ccall((:clingo_model_symbols_size, libclingo), Bool, (Ptr{clingo_model_t}, clingo_show_type_bitset_t, Ptr{Csize_t}), model, show, size)
end

function clingo_model_symbols(model, show, symbols, size)
    @wraperror ccall((:clingo_model_symbols, libclingo), Bool, (Ptr{clingo_model_t}, clingo_show_type_bitset_t, Ptr{clingo_symbol_t}, Csize_t), model, show, symbols, size)
end

function clingo_model_contains(model, atom, contained)
    @wraperror ccall((:clingo_model_contains, libclingo), Bool, (Ptr{clingo_model_t}, clingo_symbol_t, Ptr{Bool}), model, atom, contained)
end

function clingo_model_is_true(model, literal, result)
    @wraperror ccall((:clingo_model_is_true, libclingo), Bool, (Ptr{clingo_model_t}, clingo_literal_t, Ptr{Bool}), model, literal, result)
end

function clingo_model_cost_size(model, size)
    @wraperror ccall((:clingo_model_cost_size, libclingo), Bool, (Ptr{clingo_model_t}, Ptr{Csize_t}), model, size)
end

function clingo_model_cost(model, costs, size)
    @wraperror ccall((:clingo_model_cost, libclingo), Bool, (Ptr{clingo_model_t}, Ptr{Int64}, Csize_t), model, costs, size)
end

function clingo_model_optimality_proven(model, proven)
    @wraperror ccall((:clingo_model_optimality_proven, libclingo), Bool, (Ptr{clingo_model_t}, Ptr{Bool}), model, proven)
end

function clingo_model_thread_id(model, id)
    @wraperror ccall((:clingo_model_thread_id, libclingo), Bool, (Ptr{clingo_model_t}, Ptr{clingo_id_t}), model, id)
end

function clingo_model_extend(model, symbols, size)
    @wraperror ccall((:clingo_model_extend, libclingo), Bool, (Ptr{clingo_model_t}, Ptr{clingo_symbol_t}, Csize_t), model, symbols, size)
end

function clingo_model_context(model, control)
    @wraperror ccall((:clingo_model_context, libclingo), Bool, (Ptr{clingo_model_t}, Ptr{Ptr{clingo_solve_control_t}}), model, control)
end

function clingo_solve_control_symbolic_atoms(control, atoms)
    @wraperror ccall((:clingo_solve_control_symbolic_atoms, libclingo), Bool, (Ptr{clingo_solve_control_t}, Ptr{Ptr{clingo_symbolic_atoms_t}}), control, atoms)
end

function clingo_solve_control_add_clause(control, clause, size)
    @wraperror ccall((:clingo_solve_control_add_clause, libclingo), Bool, (Ptr{clingo_solve_control_t}, Ptr{clingo_literal_t}, Csize_t), control, clause, size)
end
...
function clingo_solve_handle_get(handle, result)
    @wraperror ccall((:clingo_solve_handle_get, libclingo), Bool, (Ptr{clingo_solve_handle_t}, Ptr{clingo_solve_result_bitset_t}), handle, result)
end
...
function clingo_solve_handle_model(handle, model)
    @wraperror ccall((:clingo_solve_handle_model, libclingo), Bool, (Ptr{clingo_solve_handle_t}, Ptr{Ptr{clingo_model_t}}), handle, model)
end

function clingo_solve_handle_core(handle, core, size)
    @wraperror ccall((:clingo_solve_handle_core, libclingo), Bool, (Ptr{clingo_solve_handle_t}, Ptr{Ptr{clingo_literal_t}}, Ptr{Csize_t}), handle, core, size)
end

function clingo_solve_handle_resume(handle)
    @wraperror ccall((:clingo_solve_handle_resume, libclingo), Bool, (Ptr{clingo_solve_handle_t},), handle)
end

function clingo_solve_handle_cancel(handle)
    @wraperror ccall((:clingo_solve_handle_cancel, libclingo), Bool, (Ptr{clingo_solve_handle_t},), handle)
end

function clingo_solve_handle_close(handle)
    @wraperror ccall((:clingo_solve_handle_close, libclingo), Bool, (Ptr{clingo_solve_handle_t},), handle)
end
...
function clingo_ast_copy(ast, copy)
    @wraperror ccall((:clingo_ast_copy, libclingo), Bool, (Ptr{clingo_ast_t}, Ptr{Ptr{clingo_ast_t}}), ast, copy)
end

function clingo_ast_deep_copy(ast, copy)
    @wraperror ccall((:clingo_ast_deep_copy, libclingo), Bool, (Ptr{clingo_ast_t}, Ptr{Ptr{clingo_ast_t}}), ast, copy)
end

function clingo_ast_less_than(a, b)
    @wraperror ccall((:clingo_ast_less_than, libclingo), Bool, (Ptr{clingo_ast_t}, Ptr{clingo_ast_t}), a, b)
end

function clingo_ast_equal(a, b)
    @wraperror ccall((:clingo_ast_equal, libclingo), Bool, (Ptr{clingo_ast_t}, Ptr{clingo_ast_t}), a, b)
end
...
function clingo_ast_to_string_size(ast, size)
    @wraperror ccall((:clingo_ast_to_string_size, libclingo), Bool, (Ptr{clingo_ast_t}, Ptr{Csize_t}), ast, size)
end

function clingo_ast_to_string(ast, string, size)
    @wraperror ccall((:clingo_ast_to_string, libclingo), Bool, (Ptr{clingo_ast_t}, Cstring, Csize_t), ast, string, size)
end

function clingo_ast_get_type(ast, type)
    @wraperror ccall((:clingo_ast_get_type, libclingo), Bool, (Ptr{clingo_ast_t}, Ptr{clingo_ast_type_t}), ast, type)
end

function clingo_ast_has_attribute(ast, attribute, has_attribute)
    @wraperror ccall((:clingo_ast_has_attribute, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Ptr{Bool}), ast, attribute, has_attribute)
end

function clingo_ast_attribute_type(ast, attribute, type)
    @wraperror ccall((:clingo_ast_attribute_type, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Ptr{clingo_ast_attribute_type_t}), ast, attribute, type)
end

function clingo_ast_attribute_get_number(ast, attribute, value)
    @wraperror ccall((:clingo_ast_attribute_get_number, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Ptr{Cint}), ast, attribute, value)
end

function clingo_ast_attribute_set_number(ast, attribute, value)
    @wraperror ccall((:clingo_ast_attribute_set_number, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Cint), ast, attribute, value)
end

function clingo_ast_attribute_get_symbol(ast, attribute, value)
    @wraperror ccall((:clingo_ast_attribute_get_symbol, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Ptr{clingo_symbol_t}), ast, attribute, value)
end

function clingo_ast_attribute_set_symbol(ast, attribute, value)
    @wraperror ccall((:clingo_ast_attribute_set_symbol, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, clingo_symbol_t), ast, attribute, value)
end

function clingo_ast_attribute_get_location(ast, attribute, value)
    @wraperror ccall((:clingo_ast_attribute_get_location, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Ptr{clingo_location_t}), ast, attribute, value)
end

function clingo_ast_attribute_set_location(ast, attribute, value)
    @wraperror ccall((:clingo_ast_attribute_set_location, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Ptr{clingo_location_t}), ast, attribute, value)
end

function clingo_ast_attribute_get_string(ast, attribute, value)
    @wraperror ccall((:clingo_ast_attribute_get_string, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Ptr{Ptr{Cchar}}), ast, attribute, value)
end

function clingo_ast_attribute_set_string(ast, attribute, value)
    @wraperror ccall((:clingo_ast_attribute_set_string, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Cstring), ast, attribute, value)
end

function clingo_ast_attribute_get_ast(ast, attribute, value)
    @wraperror ccall((:clingo_ast_attribute_get_ast, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Ptr{Ptr{clingo_ast_t}}), ast, attribute, value)
end

function clingo_ast_attribute_set_ast(ast, attribute, value)
    @wraperror ccall((:clingo_ast_attribute_set_ast, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Ptr{clingo_ast_t}), ast, attribute, value)
end

function clingo_ast_attribute_get_optional_ast(ast, attribute, value)
    @wraperror ccall((:clingo_ast_attribute_get_optional_ast, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Ptr{Ptr{clingo_ast_t}}), ast, attribute, value)
end

function clingo_ast_attribute_set_optional_ast(ast, attribute, value)
    @wraperror ccall((:clingo_ast_attribute_set_optional_ast, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Ptr{clingo_ast_t}), ast, attribute, value)
end

function clingo_ast_attribute_get_string_at(ast, attribute, index, value)
    @wraperror ccall((:clingo_ast_attribute_get_string_at, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Csize_t, Ptr{Ptr{Cchar}}), ast, attribute, index, value)
end

function clingo_ast_attribute_set_string_at(ast, attribute, index, value)
    @wraperror ccall((:clingo_ast_attribute_set_string_at, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Csize_t, Cstring), ast, attribute, index, value)
end

function clingo_ast_attribute_delete_string_at(ast, attribute, index)
    @wraperror ccall((:clingo_ast_attribute_delete_string_at, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Csize_t), ast, attribute, index)
end

function clingo_ast_attribute_size_string_array(ast, attribute, size)
    @wraperror ccall((:clingo_ast_attribute_size_string_array, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Ptr{Csize_t}), ast, attribute, size)
end

function clingo_ast_attribute_insert_string_at(ast, attribute, index, value)
    @wraperror ccall((:clingo_ast_attribute_insert_string_at, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Csize_t, Cstring), ast, attribute, index, value)
end

function clingo_ast_attribute_get_ast_at(ast, attribute, index, value)
    @wraperror ccall((:clingo_ast_attribute_get_ast_at, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Csize_t, Ptr{Ptr{clingo_ast_t}}), ast, attribute, index, value)
end

function clingo_ast_attribute_set_ast_at(ast, attribute, index, value)
    @wraperror ccall((:clingo_ast_attribute_set_ast_at, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Csize_t, Ptr{clingo_ast_t}), ast, attribute, index, value)
end

function clingo_ast_attribute_delete_ast_at(ast, attribute, index)
    @wraperror ccall((:clingo_ast_attribute_delete_ast_at, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Csize_t), ast, attribute, index)
end

function clingo_ast_attribute_size_ast_array(ast, attribute, size)
    @wraperror ccall((:clingo_ast_attribute_size_ast_array, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Ptr{Csize_t}), ast, attribute, size)
end

function clingo_ast_attribute_insert_ast_at(ast, attribute, index, value)
    @wraperror ccall((:clingo_ast_attribute_insert_ast_at, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_attribute_t, Csize_t, Ptr{clingo_ast_t}), ast, attribute, index, value)
end
...
function clingo_ast_parse_string(program, callback, callback_data, logger, logger_data, message_limit)
    @wraperror ccall((:clingo_ast_parse_string, libclingo), Bool, (Cstring, clingo_ast_callback_t, Ptr{Cvoid}, clingo_logger_t, Ptr{Cvoid}, Cuint), program, callback, callback_data, logger, logger_data, message_limit)
end

function clingo_ast_parse_files(files, size, callback, callback_data, logger, logger_data, message_limit)
    @wraperror ccall((:clingo_ast_parse_files, libclingo), Bool, (Ptr{Ptr{Cchar}}, Csize_t, clingo_ast_callback_t, Ptr{Cvoid}, clingo_logger_t, Ptr{Cvoid}, Cuint), files, size, callback, callback_data, logger, logger_data, message_limit)
end
...
function clingo_program_builder_begin(builder)
    @wraperror ccall((:clingo_program_builder_begin, libclingo), Bool, (Ptr{clingo_program_builder_t},), builder)
end

function clingo_program_builder_end(builder)
    @wraperror ccall((:clingo_program_builder_end, libclingo), Bool, (Ptr{clingo_program_builder_t},), builder)
end

function clingo_program_builder_add(builder, ast)
    @wraperror ccall((:clingo_program_builder_add, libclingo), Bool, (Ptr{clingo_program_builder_t}, Ptr{clingo_ast_t}), builder, ast)
end
...
function clingo_ast_unpool(ast, unpool_type, callback, callback_data)
    @wraperror ccall((:clingo_ast_unpool, libclingo), Bool, (Ptr{clingo_ast_t}, clingo_ast_unpool_type_bitset_t, clingo_ast_callback_t, Ptr{Cvoid}), ast, unpool_type, callback, callback_data)
end
...
function clingo_control_new(arguments, arguments_size, logger, logger_data, message_limit)
    control = Ref(Ptr{clingo_control}())
    @wraperror ccall((:clingo_control_new, libclingo), Bool, (Ptr{Ptr{Cchar}}, Csize_t, clingo_logger_t, Ptr{Cvoid}, Cuint, Ptr{Ptr{clingo_control_t}}), arguments, arguments_size, logger, logger_data, message_limit, control)
    return control()
end
...
function clingo_control_load(control, file)
    @wraperror ccall((:clingo_control_load, libclingo), Bool, (Ptr{clingo_control_t}, Cstring), control, file)
end

function clingo_control_add(control, name, parameters, parameters_size, program)
    @wraperror ccall((:clingo_control_add, libclingo), Bool, (Ptr{clingo_control_t}, Cstring, Ptr{Ptr{Cchar}}, Csize_t, Cstring), control, name, parameters, parameters_size, program)
end

function clingo_control_ground(control, parts, parts_size, ground_callback, ground_callback_data)
    @wraperror ccall((:clingo_control_ground, libclingo), Bool, (Ptr{clingo_control_t}, Ptr{clingo_part_t}, Csize_t, clingo_ground_callback_t, Ptr{Cvoid}), control, parts, parts_size, ground_callback, ground_callback_data)
end

function clingo_control_solve(control, mode, assumptions, assumptions_size, notify, data)
    handle = Ref(Ptr{clingo_solve_handle}())
    @wraperror ccall((:clingo_control_solve, libclingo), Bool, (Ptr{clingo_control_t}, clingo_solve_mode_bitset_t, Ptr{clingo_literal_t}, Csize_t, clingo_solve_event_callback_t, Ptr{Cvoid}, Ptr{Ptr{clingo_solve_handle_t}}), control, mode, assumptions, assumptions_size, notify, data, handle)
    return handle[]
end

function clingo_control_cleanup(control)
    @wraperror ccall((:clingo_control_cleanup, libclingo), Bool, (Ptr{clingo_control_t},), control)
end

function clingo_control_assign_external(control, literal, value)
    @wraperror ccall((:clingo_control_assign_external, libclingo), Bool, (Ptr{clingo_control_t}, clingo_literal_t, clingo_truth_value_t), control, literal, value)
end

function clingo_control_release_external(control, literal)
    @wraperror ccall((:clingo_control_release_external, libclingo), Bool, (Ptr{clingo_control_t}, clingo_literal_t), control, literal)
end

function clingo_control_register_propagator(control, propagator, data, sequential)
    @wraperror ccall((:clingo_control_register_propagator, libclingo), Bool, (Ptr{clingo_control_t}, Ptr{clingo_propagator_t}, Ptr{Cvoid}, Bool), control, propagator, data, sequential)
end

function clingo_control_is_conflicting(control)
    @wraperror ccall((:clingo_control_is_conflicting, libclingo), Bool, (Ptr{clingo_control_t},), control)
end

function clingo_control_statistics(control)
    statistics = Ref(Ptr{clingo_statistics}())
    @wraperror ccall((:clingo_control_statistics, libclingo), Bool, (Ptr{clingo_control_t}, Ptr{Ptr{clingo_statistics_t}}), control, statistics)
    return statistics[]
end
...
function clingo_control_clasp_facade(control)
    clasp = Ref(Ptr{Cvoid}())
    @wraperror ccall((:clingo_control_clasp_facade, libclingo), Bool, (Ptr{clingo_control_t}, Ptr{Ptr{Cvoid}}), control, clasp)
    return clasp[]
end

function clingo_control_configuration(control)
    configuration = Ref(Ptr{clingo_configuration}())
    @wraperror ccall((:clingo_control_configuration, libclingo), Bool, (Ptr{clingo_control_t}, Ptr{Ptr{clingo_configuration_t}}), control, configuration)
    return configuration[]
end

function clingo_control_set_enable_enumeration_assumption(control, enable)
    @wraperror ccall((:clingo_control_set_enable_enumeration_assumption, libclingo), Bool, (Ptr{clingo_control_t}, Bool), control, enable)
end

function clingo_control_get_enable_enumeration_assumption(control)
    @wraperror ccall((:clingo_control_get_enable_enumeration_assumption, libclingo), Bool, (Ptr{clingo_control_t},), control)
end

function clingo_control_set_enable_cleanup(control, enable)
    @wraperror ccall((:clingo_control_set_enable_cleanup, libclingo), Bool, (Ptr{clingo_control_t}, Bool), control, enable)
end

function clingo_control_get_enable_cleanup(control)
    @wraperror ccall((:clingo_control_get_enable_cleanup, libclingo), Bool, (Ptr{clingo_control_t},), control)
end

function clingo_control_get_const(control, name, symbol)
    @wraperror ccall((:clingo_control_get_const, libclingo), Bool, (Ptr{clingo_control_t}, Cstring, Ptr{clingo_symbol_t}), control, name, symbol)
end

function clingo_control_has_const(control, name, exists)
    @wraperror ccall((:clingo_control_has_const, libclingo), Bool, (Ptr{clingo_control_t}, Cstring, Ptr{Bool}), control, name, exists)
end

function clingo_control_symbolic_atoms(control)
    atoms = Ref(Ptr{clingo_symbolic_atoms}())
    @wraperror ccall((:clingo_control_symbolic_atoms, libclingo), Bool, (Ptr{clingo_control_t}, Ptr{Ptr{clingo_symbolic_atoms_t}}), control, atoms)
    return atoms[]
end

function clingo_control_theory_atoms(control)
    atoms = Ref(Ptr{clingo_theory_atoms}())
    @wraperror ccall((:clingo_control_theory_atoms, libclingo), Bool, (Ptr{clingo_control_t}, Ptr{Ptr{clingo_theory_atoms_t}}), control, atoms)
    return atoms[]
end

function clingo_control_register_observer(control, observer, replace, data)
    @wraperror ccall((:clingo_control_register_observer, libclingo), Bool, (Ptr{clingo_control_t}, Ptr{clingo_ground_program_observer_t}, Bool, Ptr{Cvoid}), control, observer, replace, data)
end

function clingo_control_backend(control)
    backend = Ref(Ptr{clingo_backend}())
    @wraperror ccall((:clingo_control_backend, libclingo), Bool, (Ptr{clingo_control_t}, Ptr{Ptr{clingo_backend_t}}), control, backend)
    return backend[]
end

function clingo_control_program_builder(control)
    builder = Ref(Ptr{clingo_program_builder}())
    @wraperror ccall((:clingo_control_program_builder, libclingo), Bool, (Ptr{clingo_control_t}, Ptr{Ptr{clingo_program_builder_t}}), control, builder)
    return builder[]
end
...
function clingo_options_add(options, group, option, description, parse, data, multi, argument)
    @wraperror ccall((:clingo_options_add, libclingo), Bool, (Ptr{clingo_options_t}, Cstring, Cstring, Cstring, Ptr{Cvoid}, Ptr{Cvoid}, Bool, Cstring), options, group, option, description, parse, data, multi, argument)
end

function clingo_options_add_flag(options, group, option, description, target)
    @wraperror ccall((:clingo_options_add_flag, libclingo), Bool, (Ptr{clingo_options_t}, Cstring, Cstring, Cstring, Ptr{Bool}), options, group, option, description, target)
end
